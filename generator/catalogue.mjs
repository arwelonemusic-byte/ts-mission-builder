...
// Enemy roster for Advanced AI placement (labelled groups + collapsed character
// roles per faction) — AUTO-GENERATED by tools/build-roster.mjs from
// input/roster-labels.json; see the resolvers at the end of this file.
import { ENEMY_GROUPS, ENEMY_ROLES } from "./roster.mjs";
export { ENEMY_GROUPS, ENEMY_ROLES };
...
export const K = {
  ...
  // Advanced AI placement (2026-09-21): SF waypoint slot — ground truth Operation NewOp AO.layer
  SLOTWAYPOINT_PREFAB: "{EBD91177954E8236}Prefabs/Systems/ScenarioFramework/Components/SlotWaypoint.et",
  ...
  CMP_SF_SLOTWAYPOINT: "{5EEEE11FAD8EE85C}",
...
// ---------------------------------------------------------------------------
// Advanced AI placement — roster resolvers. Elements store roster IDS
// ("USSR/USSR_Army/RifleSquad", "USSR/soviet-army/rifleman"); the emission
// resolves them to ONE prefab ref, picking among a role's/group's variants with
// a hash seeded on the element id so a regenerated mission keeps the same kit
// (mintGuid() already makes re-runs non-byte-identical, so the goal is
// gameplay stability, not byte identity). A raw "{GUID}path" ref passes
// through (CLI escape hatch, like ENEMY.vehicles[k] ?? MOD_VEHICLES[k]).
// ---------------------------------------------------------------------------

/** Deterministic pick: FNV-1a over the seed string → refs[h % n]. */
export function pickVariant(refs, seed) {
  if (!refs?.length) throw new Error("pickVariant: empty variant list");
  ...
  return refs[h % refs.length];
}

/** Factions whose roster entries an enemy may use: itself + its alias base
 * (SFS_US may place vanilla US roles — same in-game faction, deps already covered). */
export function rosterFactionsFor(enemyKey) {
  const f = FACTIONS[enemyKey];
  if (!f) throw new Error(`Unknown faction: ${enemyKey}`);
  return [enemyKey, f.aliasOf].filter(Boolean);
}

function rosterLookup(table, enemyKey, id, what) {
  for (const fk of rosterFactionsFor(enemyKey)) {
    const hit = (table[fk] ?? []).find((e) => e.id === id);
    if (hit) return hit;
  }
  throw new Error(`Unknown ${what} ${id} for enemy ${enemyKey}`);
}

/** Group ref for an advanced element: roster group id or raw ref. */
export function resolveAdvancedGroup(enemyKey, groupRef, seed) {
  if (typeof groupRef === "string" && groupRef.startsWith("{")) return groupRef;
  return pickVariant(rosterLookup(ENEMY_GROUPS, enemyKey, groupRef, "roster group").refs, seed);
}

/** Character ref for a static soldier: roster role id, `{ refs: [...] }`, or raw ref. */
export function resolveAdvancedRole(enemyKey, roleRef, seed) {
  if (typeof roleRef === "string" && roleRef.startsWith("{")) return roleRef;
  if (roleRef && typeof roleRef === "object") return pickVariant(roleRef.refs, seed);
  return pickVariant(rosterLookup(ENEMY_ROLES, enemyKey, roleRef, "roster role").refs, seed);
}
